...
function Collection() {
    ...
    useEffect(() => {
        if (coll) {
            genericGet('/api/artwork/collection/' + coll.id).then(
                (response: Artwork[]) => {
                    let filteredResponse: Artwork[] = response.filter(
                        (elem: Artwork) => elem.appId !== null
                    )
    ...
    const getArtwork = async (allArtworks: Artwork[]) => {
        ...
        for (const elem of allArtworks) {
            ...
            globalState.forEach((item) => {
                const key: string =
                    nameMapping[Buffer.from(item['key'], 'base64').toString()]
                if (!key) {
                    return
                }
    ...
    const buyArtwork = async (art: Artwork): Promise<void> => {
        ...
        const psResp: ProfitSplit[] = await genericGet(
            '/api/profitSplit/collection/' + art.origColl.id
        )
        ...
        if (coll.collType === CollectionType.SALE && asset.asaPrice) {
            ...
            buy = buyAsset(
                ...
                asset.asaPrice + 2000,
                asset.escrowAddress,
                getApplicationAddress(art.appId)
            )
            ...
            const price: number | bigint =
                ((asset.startTime + asset.duration - current) *
                    (asset.startPrice - asset.endPrice)) /
                    asset.duration +
                asset.endPrice +
                2000
            buy = buyAsset(
        ...
        await callApplicationSign(
            art.appId,
            adminAddr,
            algosdk.OnApplicationComplete.NoOpOC,
            [MAKE_PAYMENTS, ...percents],
            undefined,
            wallets,
            true
        )
    ...
    const buyShuffle = async (): Promise<void> => {
        ...
        while (true) {
            const response = await genericGet(
                '/api/collection/soldMask/' + coll.id
            )
        ...
        artwork?.forEach((elem: Artwork) => {
            ...
            listings.push(
                <div key={elem.id}>
                    ...
                    {user &&
                        user.walletAddress &&
                        currAsset.asaOwner === user.walletAddress && (
                            <TwineButton
                                action={() => {
                                    stopSellOffer(elem)
                                }}
