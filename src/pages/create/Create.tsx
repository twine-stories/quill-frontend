...
function Create() {
    ...
    const [smartContractInfo, setSmartContractInfo] =
        useState<Record<number, AssetInfo>>()

    const [contractType, setContractType] = useState<CollectionType>(
        CollectionType.SALE
    )
    ...
    useEffect(() => {
        if (user) {
            ...
            worksGetByCreator(user.walletAddress).then(
                (response: Work[] | null) => {
                    if (response) {
    ...
    const genContract = async (
        ...
        const id: number = await createApplication(
            data['approval'],
            data['clear'],
            data['global_uints'],
            data['global_byte_slices'],
            data['local_uints'],
            data['local_byte_slices'],
            [
                decodeAddress(user.walletAddress).publicKey,
                decodeAddress(adminAddr).publicKey,
            ],
            [assetId]
        )
        const escrowProgram: string = await getEscrowProgram(
        ...
        if (escrowProgram) {
            const escrowAddress: string =
                await escrowProgramToAddress(escrowProgram)
            await Promise.all([
                callApplicationSign(
                    id,
                    adminAddr,
                    algosdk.OnApplicationComplete.NoOpOC,
                    [INIT_ESCROW, decodeAddress(escrowAddress).publicKey],
                    undefined,
                    undefined,
                    true
                ),
    ...
    const confirmNFTs = async () => {
        let nftList: number[] = []
        const checkboxes: HTMLCollectionOf<Element> =
        ...
        const collabAddrs: HTMLCollectionOf<Element> =
            document.getElementsByClassName('bottomCollab')
        const collabVals: HTMLCollectionOf<Element> =
            document.getElementsByClassName('topRightCollab')
        let addrs: string[] = []
        let vals: number[] = []
    ...
    const makeSellOffer = async (): Promise<void> => {
        ...
        for (const id in smartContractInfo) {
            ...
            if (info.price && contractType === CollectionType.SALE) {
                ...
                info.endPrice &&
                info.duration &&
                contractType === CollectionType.REV_AUCTION
            ) {
                ...
                txns.push(
                    callApplication(
                        ...
                        user.walletAddress,
                        algosdk.OnApplicationComplete.NoOpOC,
                        [MAKE_SELL_OFFER, startPrice, endPrice, duration],
                        [assetId]
                    )
                )
            } else {
        ...
        const work: HTMLInputElement = document.getElementById(
            'works'
        ) as HTMLInputElement
        ...
        const coll: NFTCollection = await collectionCreateWithArt(
            collection,
            artworks
        )
        ...
        <div className="create">
            ...
            {env === 'dev' && (
                <div>
                    ...
                    <div>
                        <TwineButton
                            name="Generate Contract(s)"
                            action={confirmNFTs}
                        />
                        <TwineButton
                            name="Post NFT(s) for Sale"
                            enabled={enableSell}
                            action={(e) => makeSellOffer()}
                        />
                    </div>
                </div>
            )}
