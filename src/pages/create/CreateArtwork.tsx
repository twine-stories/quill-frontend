...
function CreateArtwork() {
    ...
    const reset = () => {
        ...
        (document.getElementById('asset-description') as HTMLInputElement).value = '1';
    }

    const mintNft = async () => {
        const unitName: HTMLInputElement = document.getElementById('nickname') as HTMLInputElement;
        const assetName: HTMLInputElement = document.getElementById('asset-name') as HTMLInputElement;
        const numAssets: HTMLInputElement = document.getElementById('num-assets') as HTMLInputElement;
    ...
    const inputFieldsAreValid = () => {
        const unitName: HTMLInputElement = document.getElementById('nickname') as HTMLInputElement;
        const assetName: HTMLInputElement = document.getElementById('asset-name') as HTMLInputElement;
        const assetDescription: HTMLInputElement = document.getElementById('asset-description') as HTMLInputElement;
        const numAssets: HTMLInputElement = document.getElementById('num-assets') as HTMLInputElement;
    ...
    return (
        <div>
            ...
            <Grid>
                ...
                <Grid container direction='column' rowSpacing={3} sx={{marginLeft: '30px'}}>
                    <Grid xs={11}>
                    ...
                    <Grid container direction='column' alignItems='center' justifyContent='space-around' xs={11} id='create-art-upload-wrapper'>
                        <Grid xs={12}>
                            <Typography level='h3' color='purple'>Art</Typography>
                        </Grid>
                        <Grid container alignItems='center' justifyContent='center' id='create-art-image-upload'>
                            {assetImg.preview ? 
                                <img
                    ...
                    />
                    <Grid xs={11}>
                        <TwineButton name={uploadDataLoading ? <CircularProgress color='darkpurple' variant='plain' /> : 'Upload Data'} action={async (e) => {
                            const assetDescription: HTMLInputElement = document.getElementById('asset-description') as HTMLInputElement;
                            ...
                            if (inputFieldsAreValid() && assetImg.file) {
                                ...
                                const mintNumber = await genericGet('/api/algo/mint-number');
                ...
                <MintNftPopup isOpen={openNftPopup} onClose={() => setOpenNftPopup(false)} mintNft={() => mintNft()} loading={mintNftLoading} />
