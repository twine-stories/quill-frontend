...
function CreateChapter(props: CreateChapterProps) {
  ...
  useEffect(() => {
    if (user && props.edit && chapter && !populatedForEdit) {
      ...
      genericGet("/api/profitSplit/episode/" + chapter.id).then(
  ...
  function onAddImageButtonClick(defaultImgSrc = "") {
    ...
    setInputList(
      inputList.concat(<InputListItem key={counter} counter={counter} />)
    );
    setCounter(counter + 1);
  }
        ...
        leftComponent={
          <div>
            <Typography level="h2" color="purple">
              {props.edit ? "Edit Chapter" : "Create Chapter"}
            </Typography>
            <div
              id="chapter-content"
              style={{ display: view ? null : "none", width: "100%" }}
            >
              <Box
                sx={{
                  py: 1,
              ...
              >
                <Grid sx={{ width: "100%" }}>{preview}</Grid>
              </Box>
            </div>
            ...
            <div style={{ display: view ? "none" : null }}>
              ...
              >
                ...
                {(!props.edit || chapter) && (
                  <TwineInput
                    defaultValue={props.edit ? chapter["title"] : ""}
                    id="title"
                    label="Chapter Title"
                ...
                {(!props.edit || chapter) && (
                  <TwineInput
                    defaultValue={
                      props.edit ? chapter["endOfChapterMessage"] : ""
                    }
                    id="endOfChapterMessage"
                    label="End of Chapter Message (Optional)"
              ...
              >
                ...
                >
                  ...
                  <TwineButton
                    ...
                    enabled={collaborators.length < MAX_COLLABORATORS}
                    sx={{ marginTop: "20px" }}
                    action={(e) => {
          ...
          >
                    ...
                    src={
                      cover.preview
                        ...
                        : "https://" +
                          ...
                          ".s3.amazonaws.com/" +
                          COVER_PATH +
                          (chapter ? chapter.cover : cover.name)
                    }
                    alt=""
                    onClick={() =>
                      setCover({
            ...
            {(!props.edit || chapter) && (
              <Typography
                ...
                endDecorator={
                  <Switch
                    defaultChecked={props.edit ? chapter["mature"] : false}
                    id="mature"
                    sx={{ ml: 1 }}
                  />
                }
            ...
            >
              <Checkbox
                defaultChecked={props.edit}
                color="purple"
                sx={{ marginRight: "20px", marginBottom: "2px" }}
                size="sm"
                label=""
                slotProps={{
                  input: {
              ...
              <Typography fontSize={"14px"} level="h6" color="white">
                I verify this work is mine and follows the{" "}
                <a
                  id="comm-guidelines"
                  href="https://twine-legal.s3.amazonaws.com/COMMUNITY_POLICY_AND_UPLOADING_GUIDELINES.pdf"
                  target="_blank"
                >
      ...
      return (
        <>
          <TwineButton
            name={
              uploading ? (
                <CircularProgress color="darkpurple" variant="plain" />
              ) : (
                "Save Chapter"
              )
            }
        ...
        if (!display) {
          ...
          compoundedElements.push(
            <AspectRatio
              ...
              sx={{ my: 2 }}
            >
              <img src={content["preview"]} loading="lazy" alt="" />
            </AspectRatio>
          );
        }
      } else if (content !== "") {
  ...
  async function checkCollaborators(): Promise<Map<User, number>> {
    const collabUsernames: HTMLCollectionOf<Element> =
      document.getElementsByClassName("usernameTopLeftCollab");
    const collabValues: HTMLCollectionOf<Element> =
  ...
  async function postEpisode(published: boolean, currentChapter?: Episode) {
    ...
    if (title.value.includes("/")) {
      setErrorMessage(
        "Sorry, there cannot be any backslashes in the chapter title."
      );
      setOpenError(true);
      return;
    }
    ...
    const mature: HTMLInputElement = document.getElementById(
      "mature"
    ) as HTMLInputElement;

    const guidelines: HTMLInputElement = document.getElementById(
      "guidelines"
    ) as HTMLInputElement;
    if (!guidelines.checked) {
      setErrorMessage(
        "Please read and check the Community Guidelines box before submitting."
      );
      setOpenError(true);
      return;
    }

    const endOfChapterMessage: HTMLInputElement = document.getElementById(
      "endOfChapterMessage"
    ) as HTMLInputElement;
    ...
    if (currentChapter && currentChapter["episodeNumber"] !== -1 && published) {
      ...
      const totalPubChapters = await genericGet(
        "/api/episode/numpublished/work/url/" + work.url
      );
    ...
    if (
      currentChapter &&
      currentChapter["episodeNumber"] !== -1 &&
      !published
    ) {
    ...
    const url: string =
      work.url + "_" + title.value.replace(/\s/g, "-").toLowerCase();
    if (
      title.value &&
      !title.value.includes("/") &&
      guidelines.checked &&
      profitSplitMap &&
      (cover.name || (chapter && chapter.cover)) &&
      allContent.length > 0
    ) {
      ...
      setUploading(true);
      try {
        const response: number = await genericPost(
          "/api/episode/" + urlModifier,
          newEpisode
        );
        if (response) {
          if (cover.file) {
    ...
    setErrorMessage(
      "Please ensure you have filled out all required fields and checked the Community Guidelines box before submitting."
    );
    setOpenError(true);
  }
}
